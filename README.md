...
- 📊 **詳細なログ出力**: 処理状況を時系列でログファイルに出力（JST対応）
...
- 🕘 **JST対応**: ログ・メタデータが日本標準時で統一表示
- 📝 **メタデータ拡張**: ファイル最終更新日時の自動設定
...
### v2.3.0 (2025-09-06)
- 🕘 **JST対応**: ログタイムスタンプとメタデータが日本標準時で統一表示
- 📝 **メタデータ拡張**: ファイル最終更新日時（last_update_date）の自動設定
- 🌐 **タイムゾーン一貫性**: 全機能でJSTタイムゾーンが統一

### v2.2.0
- 🧩 **チャンク設定機能**: 文書分割設定のカスタマイズ対応
- 🖥️ **クロスプラットフォーム**: Windows/Unix パス正規化対応

### v2.1.0
- 📁 **10種ファイル形式対応**: .pdf、.pptx、.ppt、.xls、.doc、.xlsm形式を追加

### v2.0.0
- 🔍 **ファイル更新検知**: SHA256ハッシュベースの変更検知機能
- 💾 **自動バックアップ**: 変換前ファイルの安全な保存機能
